...
var 常用 = /*#__PURE__*/function () {
  ...
  _createClass(常用, null, [{
    key: "\u662F\u5426\u4EE5\u5B57\u7B26\u4E32\u7ED3\u675F",
    value: function 是否以字符串结束(字符串, 列表) {
      // endsWith
      var ret = false;

      var _iterator = _createForOfIteratorHelper(列表),
          _step;
  ...
  _createClass(狗屁库, [{
    ...
    value: function 生成() {
      ...
      var 这是啥我也不知道 = function 这是啥我也不知道(主要的, 次要的) {
        return 主要的.length <= 0 ? _this.最后一项(次要的) : 主要的;
      }; // 申密函数
      ...
      while (this.总长(文章) < 长度 && (!this.选项["能写少就写少"] || this.总长(文章) + 段缓冲.length < 长度) || this.选项.v3语法改进 && this.常用.是否以字符串结束(这是啥我也不知道(段缓冲, 文章).trim(), ["：", ":", ",", "，"])) {
        ...
        if (分支 < 5 && (!this.选项.v3语法改进 || !(段缓冲.length <= 0 || this.常用.是否以字符串结束(段缓冲.trim(), [",", "，", "、"])))) {
          ...
          if (!this.选项.有概率不添加后面垫话 || this.随机常用.取随机数(10) != 1) {
        ...
        if (this.能加段(段缓冲) || 类型 == "新行") {
          文章 = this.加段(文章, 起段 + 段缓冲);
          段缓冲 = "";
        }
      }

      if (段缓冲 != "") 文章 = this.加段(文章, 起段 + 段缓冲);
      段缓冲 = "";
