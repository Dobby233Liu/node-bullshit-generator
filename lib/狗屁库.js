...
var 狗屁库 = /*#__PURE__*/function () {
  ...
  _createClass(狗屁库, [{
    ...
    value: function 生成() {
      ...
      while (this.总长(文章) < 长度) {
        // || (this.选项.v3语法改进 && (this.最后一项(文章).trim().endsWith("：") || 文章.trim().endsWith(":") || 文章.trim().endsWith(",") || 文章.trim().endsWith("，")))) {
        var 缓冲 = "";
        类型 = "忽略";
        var 分支 = Math.floor(Math.random() * 100);

        if (分支 < 5 && (!this.选项.v3语法改进 || !(段缓冲.length <= 0 || 段缓冲.trim().endsWith(",") || 段缓冲.trim().endsWith("，")))) {
      ...
      if (!this.选项["返回段落列表"]) 文章 = 文章.map(function (x) {
        return 起段 + x;
      }).join("\n");
      return 文章;
    }
