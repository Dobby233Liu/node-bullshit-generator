...
function 生成狗屁(主题 = "学生会退会", 名人名言 = [
	 ...
	 * @param {Array} 列表
	 */
	var 洗牌遍历 = function (列表) {
		let 池 = [];
		池 = 池.concat(列表);
		池 = 池.concat(列表);
		return 洗牌(池);
	}
	var 瞎选一个 = function (数组) { // 可能不同
		let max = 数组.length;
		let min = 0;
	...
	while (tmp.length < 6000 * 主题.length) {
		分支 = Math.floor(Math.random() * 100)
		if(分支 < 5 && tmp2 != "    " && !tmp2.endsWith(",") && !tmp2.endsWith("，")) tmp2 = 另起一段(能否以句号结束(tmp2.trim()));
