...
class 狗屁库 {
    ...
    生成(主题 = "学生会退会", 文章长度 = 6000 * 主题.length, 名人名言 = require("./dict/default/名人名言"), 废话 = require("./dict/default/废话")(主题), 前面垫话 = require("./dict/default/前面垫话.js"), 后面垫话 = require("./dict/default/后面垫话"), _选项 = this.defaultOpts) {
        ...
        var 分支 = 0; // quick fix
        maketmp: while (tmp.length < 文章长度 || (this.选项.v3语法改进 ? (tmp.trim()
                .endsWith("：") || tmp.trim()
                .endsWith(":") || tmp.trim()
                .endsWith(",") || tmp.trim()
                .endsWith("，")) : false)) {
            分支 = Math.floor((this.选项.LCG随机数机 ? this.LCG.来个小小数() : Math.random()) * 100);
            if (分支 < 5 && (this.选项.v3语法改进 ? (!tmp.endsWith(sctStart) && !tmp2.trim()
                    .endsWith(",") && !tmp2.trim()
                    .endsWith("，")) : true)) {
            ...
            } else if ((this.选项.分支大于24时加废话 ? 分支 > 24 : true) && (this.选项.有时不加废话 ? !(
                    (Math.floor(
                        (this.选项.LCG随机数机 ? this.LCG.来个小小数() : Math.random())) * 80) & 7 == 0) : true)) {
